...
import com.l2jfrozen.sperocoin.core.SperocoindException;
import com.l2jfrozen.sperocoin.core.HttpException;
import com.l2jfrozen.sperocoin.core.RPCErrorCode;
import com.l2jfrozen.sperocoin.response.BaseResponse;
import com.l2jfrozen.sperocoin.response.Error;
import com.l2jfrozen.sperocoin.response.Fallback;

/**
 *  @author Sebastian Dziak {@literal (sebastian.dziak@sulacosoft.com)}
 *  @author Nicola Atzei
 */
public class SperocoindApiHandler implements InvocationHandler {
	...
	public SperocoindApiHandler(String host, int port, String protocol, String uri, String username, String password) {
		this.uri = uri;
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		try (CloseableHttpResponse response = httpClient.execute(targetHost, httpPost, context);) {
			...
			if (error != null)
				throw new SperocoindException(error.getMessage(), RPCErrorCode.fromCode(error.getCode()));
			...
			throw new SperocoindConnector4JException(e.getMessage(), -1);
		}
	}
		...
		if (statusCode != HttpStatus.SC_OK && statusCode != HttpStatus.SC_INTERNAL_SERVER_ERROR) {
			if (statusCode == HttpStatus.SC_UNAUTHORIZED)
				throw new HttpException(String.format(
						"Sperocoind JSON-RPC HTTP error (Probably an incorrect username or password). HTTP Status-Code %s",
						statusCode), statusCode);
			throw new HttpException(String.format("Sperocoind JSON-RPC HTTP error. HTTP Status-Code %s", statusCode),
					statusCode);
		}
	}
}
